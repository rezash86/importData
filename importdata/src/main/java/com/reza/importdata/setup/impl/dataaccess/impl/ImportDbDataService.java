...
public class ImportDbDataService implements IImportDbDataService {
	...
	public int createData(MarketPrice marketprice) {
		String SQL = "insert into miso_market_price_five_minutes(originaldatetime, hubname, lmp, loss, congestion) "
				+ " values(?, ?, ?, ?, ?) ";
		return jdbcTemplateObject.update(SQL, Timestamp.valueOf(marketprice.getOriginalDateTime()),
				marketprice.getHubname(), marketprice.getLmp(), marketprice.getLoss(), marketprice.getCongestion());
	}
	...
	public boolean dataExists(LocalDateTime marketPriceTime) {
		...
		List<MarketPrice> marketPrices = jdbcTemplateObject.query(SQL,
				new Timestamp[] { Timestamp.valueOf(marketPriceTime) }, new MarketPriceMapper());
	...
	public void deleteData(LocalDateTime marketPriceTime) {
		String SQL = "delete from miso_market_price_five_minutes where originaldatetime=? ";
		jdbcTemplateObject.update(SQL, Timestamp.valueOf(marketPriceTime));
	}
